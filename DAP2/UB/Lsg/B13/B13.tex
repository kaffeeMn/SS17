...
\subsubsection \
Wir definieren zunaechst eien Rekursionsgleichung, die zum Erreichen des Blocks (i,j) die minimale Distanz ausgibt.\\
$
E'(i,j) = \begin{cases}
    \ue               &, i=0\\
    0                 &, j=0\\
    $min\{E(i-1, j-1), E(i,j-1)\} + S(i, j)$, sonst\\
\end{cases}\\
$
Nun, da wir die kuerzeste Distanz zu einem Block berechnen koennen, koennen wir auch fuer jedes j die kuerzeste Distanz
bis einschliesslich i berechnen.\\
\subsubsection \
RUN(S):\\
...
\tab \tab \tab E[i][j] \lA $min\{S[i-1,j], S[i,j]\} + E[i][j-1]$\\
\tab result \lA \ue\\
